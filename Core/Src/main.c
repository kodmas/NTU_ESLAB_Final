...
int Recognize_and_output(const char *word)
{
    if(strcmp(word, ON) == 0)
    {
        return 1;
...
int main(void){
    ...
    while(1)
    {
        ...
        // Run inference
        nbatch = ai_speech_commands_model_run(speech_commands_model, &ai_input[0], &ai_output[0]);

        execution_time_stop(2);

        if(nbatch != 1)
        {
            usart_buffer_length = sprintf(usart_buffer, "Error: could not run inference\r\n");
